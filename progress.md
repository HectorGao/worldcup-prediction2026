...
| Frontend syntax after card-grid heatmap UI | `node --check src/main.js` | No syntax errors after responsive card and heatmap changes | Passed | ✓ |
| Focused suite after card-grid heatmap UI | `.venv/bin/python -m pytest tests/test_service_api.py tests/test_prediction_core.py -q` | Score heatmap exposes handicap probabilities and final matrix regions | 24 passed, 1 warning | ✓ |
| Full backend suite after card-grid heatmap UI | `.venv/bin/python -m pytest -q` | All regression tests pass | 51 passed, 1 warning | ✓ |
| API smoke after final-matrix heatmap | `POST /api/predict/lyihub-54327932` via TestClient | Final prediction exposes full calibrated matrix and handicap probabilities | 65 matrix rows, 65 heatmap rows, handicap `+1`, let-win/draw/loss probabilities present | ✓ |
| Browser smoke after card-grid heatmap UI | `http://127.0.0.1:8000/` with Codex in-app browser | Today page uses a responsive match-card grid and detail shows full 0-7 handicap heatmap | 6 matches, full-width Today panel, 4 desktop card columns, 64 heat cells, `7-7` visible, let-win/draw/loss totals with market probabilities visible | ✓ |
| Browser smoke after heatmap v2 copy fix | `http://127.0.0.1:8000/?heatmap=2` with Codex in-app browser | Detail heatmap labels match the actual 0-7 grid and 8+ tail bucket | Brazil vs Japan detail: 64 cells, `0-7 热力图`, `8+ tail`, let-win/draw/loss totals visible | ✓ |
| Sporttery mobile page re-check | `https://m.sporttery.cn/mjc/jsq/zqspf/` in browser | Extract currently purchasable World Cup SPF/RQSPF window | 9 matches: 周一074-076, 周二077-079, 周三080-082 | ✓ |
| Frontend syntax after live-refresh wiring | `node --check src/main.js` | No syntax errors after refresh button update | Passed | ✓ |
| Focused suite after live-refresh wiring | `.venv/bin/python -m pytest tests/test_service_api.py tests/test_prediction_core.py -q` | Current 9-match Sporttery window and prediction core pass | 24 passed, 1 warning | ✓ |
| Full backend suite after live-refresh wiring | `.venv/bin/python -m pytest -q` | All regression tests pass | 51 passed, 1 warning | ✓ |
| API smoke after service restart | `GET /api/matches?date=2026-06-29` | Today page data returns current purchasable Sporttery window | 9 matches, dates `2026-06-30/2026-07-01/2026-07-02`, updated SPF/RQSPF odds visible | ✓ |
| Refresh endpoint smoke | `POST /api/refresh/current?date=2026-06-29` | Sync completed scores and refresh odds window | lyihub 104 matches synced; Sporttery live endpoint returned 403 so snapshot fallback updated 9 matches | ✓ |
| Browser refresh-button smoke | Click top refresh button on `http://127.0.0.1:8000/` | User-facing refresh updates completed scores and purchasable odds window | Status: `同步完成：完赛比分已更新，竞彩网快照赔率 9 场，今日页 9 场。` | ✓ |
| Frontend syntax after detail scroll fix | `node --check src/main.js` | No syntax errors after cache-bust and CSS scroll edits | Passed | ✓ |
| Browser detail-scroll smoke | `http://127.0.0.1:8000/?scroll_verify=7` with Codex in-app browser | Single-match detail uses document-level scrolling and reaches the bottom | `#detail` static, max-height none, prediction overflow visible, page scrolled to 4523px bottom, 64 heat cells visible | ✓ |
| Browser wide detail-scroll v2 smoke | `http://127.0.0.1:8000/?cache_bust=detail_scroll_v2#detail` | Wide detail page loads fresh assets, does not inherit panel clipping, and reaches page bottom | assets `detail-scroll-v2`, `#detail` overflow visible, `#prediction-detail` overflow visible, page scrolled to 5046px bottom, 64 heat cells visible | ✓ |
| Full backend suite after detail-scroll v2 | `.venv/bin/python -m pytest -q` | Cache header middleware and style changes do not break regressions | 51 passed, 1 warning | ✓ |
| HTTP cache header smoke | `curl -sS -D - http://127.0.0.1:8000/` | Homepage disables cache and returns latest asset version | `cache-control: no-store`, `detail-scroll-v2` CSS/JS present | ✓ |
| Chrome extension diagnosis for wide detail bug | Codex Chrome plugin checks | Official Chrome debugging path either connects or reports concrete blocker | Stable Chrome app installed, extension/native host valid, but Chrome launch failed with `kLSNoExecutableErr`; extension backend unavailable | ⚠ |
| Frontend syntax after collapsible detail cards | `node --check src/main.js` | No syntax errors after detail autoload and collapse controls | Passed | ✓ |
| Focused service suite after collapsible detail cards | `.venv/bin/python -m pytest tests/test_service_api.py -q` | API/service tests still pass | 8 passed, 1 warning | ✓ |
| Wide detail collapse smoke | `http://127.0.0.1:8000/?verify_collapse=3#detail` at 1680x900 | `#detail` auto-loads a prediction and long cards can collapse | loaded Brazil vs Japan, 9 toggles, score card 1052px -> 56px, roster card 253px -> 70px, page height 3971px -> 2792px, bottom reachable | ✓ |
| Full backend suite after collapsible detail cards | `.venv/bin/python -m pytest -q` | All regressions pass after UI changes | 51 passed, 1 warning | ✓ |
| HTTP asset v3 smoke | `curl -sS -D - http://127.0.0.1:8000/` | Homepage returns latest collapse assets and no-store headers | `detail-collapse-v3` CSS/JS, `cache-control: no-store` | ✓ |
...
| 2026-06-29 | Chrome extension backend was unavailable (`extension` not listed) | 1 | Followed plugin troubleshooting, then used the available Codex in-app browser for localhost UI verification. |
| 2026-06-29 | Detail page initially reused stale analysis prediction for the heatmap | 1 | Kept analysis text but forced detail rendering to use the freshly generated `POST /api/predict` payload. |
| 2026-06-29 | Sporttery backend JSON endpoint returned HTTP 403 during refresh | 1 | Kept live attempt in `/api/refresh/current`, then falls back to the browser-verified current 9-match snapshot. |
| 2026-06-29 | Stored prediction payloads preserved older odds after fixture odds changed | 1 | Today list now derives `odds_markets` and `lottery_market` directly from current fixture market fields before using prediction cache. |
| 2026-06-29 | Heatmap rendered correctly but still had stale `0-4` and `7+ tail` copy | 1 | Updated detail copy to `0-7 热力图`, changed tail labels to `8+ tail`, bumped static asset version, and restarted FastAPI. |
| 2026-06-29 | Single-match detail page could not scroll far enough to show all content | 1 | Removed the desktop sticky/max-height internal-scroll container, made `#prediction-detail` visible overflow, bumped static asset version, and verified page-level scrolling reaches the bottom. |
| 2026-06-29 | Widened browser still used old `20260629-live-refresh` assets and inherited `.panel { overflow: hidden; }` on `#detail` | 1 | Added explicit `overflow: visible !important` to `.detail-panel` and `#prediction-detail`, bumped assets to `detail-scroll-v2`, and added no-store headers for `/` and `/src/*`. |
| 2026-06-29 | Directly opening `#detail` in a wide viewport showed an empty 560px detail panel and hidden match buttons from other tabs, which looked like a folded detail page | 1 | Added detail autoload for the first current match and collapsible prediction cards; long sections can now be collapsed without losing the selected prediction. |
| 2026-06-29 | User-requested stable Chrome debugging could not attach | 1 | Chrome checks showed extension and native host installed/enabled, but launching `/Applications/Google Chrome.app` failed with `kLSNoExecutableErr`; used the available controlled browser for UI verification and recorded the blocker. |
